...
19. `RivalsCore.luau` — `PlayerDataManager.new()` returned a fresh table every call. CLAUDE.md fix #5 cached the instance on `Server.new()`, but two other call sites still did `PlayerDataManager.new()` directly: `CombatSystem:Die` (Kills/Deaths increments) and `ClientController:EquipWeapon` (WeaponsOwned lookup). Each got its own empty cache — **every Kill/Death increment vanished into an orphan table that nobody else could read**. Fix: make `PlayerDataManager.new()` a true singleton (module-level `_PdmInstance` cache) so every call site shares one cache. Surfaced by T3-Leader agent.
20. `Server.server.luau` — Server had **no Heartbeat loop**. `GameModeManager:Update` (round timer countdown, EndRound trigger), `CombatSystem:Update` (knockback decay, future armor regen) were never invoked. The 120s round timer just sat there; knockback would persist forever. Added a single `RunService.Heartbeat:Connect` in `Server:Start` that fans out to `GameMode:Update(dt)` (if active) and each `PlayerCombat[player]:Update(dt)`. Wrapped in `debug.profilebegin("RivalsServerTick")` for MicroProfiler. Surfaced by T3-M7 agent.
21. `RivalsCore.luau` — `WeaponSystem:UpdateSpread` used `self.SpreadOffset = Vector2.new(...)` (overwrite) where the math clearly intended `+=` (accumulate). Sustained fire stayed as accurate as the first shot — the `SpreadOffset.Magnitude * 0.1` accumulator was permanently 0. Switched to `self.SpreadOffset = self.SpreadOffset + ... * 0.3` and added `WeaponSystem:DecaySpread(dt)` for between-shot recovery. Surfaced by T1-M3 agent.
